...
class GeodesicSphere():
  ...
  def locate_duplicate_vertices(self):
    ...
    for i in range(len(self.unprojected_vertices)):
        replace_dict[i+1] = []
    _, indices, inverse = np.unique(np.array(self.unprojected_vertices).round(decimals=4), return_index=True, return_inverse=True, axis=0)
    for i in range(len(self.unprojected_vertices)):
      first = indices[inverse[i]]
      if i != first:
        replace_dict[i+1].append(first+1)
